...
public interface ThongKeDoanhThuRepository extends JpaRepository<HoaDon, Integer> {
    ...
    @Query(value = """
            ...
            WHERE hd.NgayIn BETWEEN :tuNgay AND :denNgay
              AND (:maNhanVien IS NULL OR dh.MaNhanVien = :maNhanVien)
              AND (hd.TrangThai IS NULL OR LOWER(hd.TrangThai) NOT LIKE '%hủy%')
            """, nativeQuery = true)
    ...
    @Query(value = """
            SELECT
                CASE
                    WHEN :kieuThongKe = 'NAM' THEN DATE_FORMAT(hd.NgayIn, '%Y')
                    WHEN :kieuThongKe = 'THANG' THEN DATE_FORMAT(hd.NgayIn, '%Y-%m')
                    ELSE DATE_FORMAT(hd.NgayIn, '%Y-%m-%d')
                END AS thoiGian,
                COALESCE(SUM(hd.TongTien), 0) AS doanhThu,
                COUNT(DISTINCT hd.MaDonHang) AS soDonHang
            FROM hoadon hd
            JOIN donhang dh ON dh.MaDonHang = hd.MaDonHang
            WHERE hd.NgayIn BETWEEN :tuNgay AND :denNgay
              AND (:maNhanVien IS NULL OR dh.MaNhanVien = :maNhanVien)
    ...
    @Query(value = """
            SELECT
                ...
                COALESCE(SUM(ct.SoLuong * ct.GiaTien), 0) AS doanhThu
            FROM hoadon hd
            JOIN donhang dh ON dh.MaDonHang = hd.MaDonHang
            JOIN chitietdonhang ct ON ct.MaDonHang = hd.MaDonHang
    ...
    @Query(value = """
            SELECT
                COALESCE(NULLIF(hd.PhuongThucThanhToan, ''), 'Chưa cập nhật') AS phuongThucThanhToan,
    ...
    @Query(value = """
            SELECT
                COALESCE(SUM(ct.SoLuong * ct.GiaTien), 0) AS tongDoanhThu,
                COUNT(DISTINCT hd.MaHoaDon) AS tongHoaDon,
                COUNT(DISTINCT hd.MaDonHang) AS tongDonHang,
                CASE
                    WHEN COUNT(DISTINCT hd.MaDonHang) = 0 THEN 0
                    ELSE COALESCE(SUM(ct.SoLuong * ct.GiaTien), 0) / COUNT(DISTINCT hd.MaDonHang)
