<template>
  <div class="chi-tiet-tin-tuc-page">
    ...
    >
      ...
      <!-- Nội dung đã được DOMPurify làm sạch -->
      <section
...
/*
 * Làm sạch nội dung HTML trước khi hiển thị.
 * Ngăn script hoặc mã độc được lưu trong nội dung bài viết.
 */
const safeArticleContent = computed(() => {
  return DOMPurify.sanitize(
      String(tinTuc.value?.noiDung || ""),
      {
        USE_PROFILES: {
          html: true
        },
 ...
 *
 * Xử lý trường hợp:
 * "5 điề u gia đình cầ n chuẩn bị"
 *
 * thành:
 * "5 điều gia đình cần chuẩn bị"
 */
const normalizedTitle = computed(() => {
  return String(tinTuc.value?.tieuDe || "")
      .normalize("NFC")
      .replace(/[\u200B-\u200D\u2060\uFEFF]/g, "")
      .replace(/\s+/g, " ")
      .replace(
...
const getChiTietTinTuc = async () => {
  ...
  if (!id) {
    ...
    loading.value = false;
    errorMessage.value =
        "Không xác định được mã bài viết.";
    return;
  }
  ...
  try {
    ...
    if (error.response?.status === 404) {
      errorMessage.value =
          "Bài viết không tồn tại, chưa được xuất bản hoặc đã bị xóa.";
    } else if (error.response?.status === 403) {
      errorMessage.value =
          "Bạn không có quyền xem bài viết này.";
    } else if (!error.response) {
      errorMessage.value =
          "Không kết nối được tới máy chủ. Hãy kiểm tra backend đang chạy ở cổng 8080.";
    } else {
      errorMessage.value =
...
/*
 * Dùng ảnh mặc định nếu ảnh đại diện bị lỗi.
 */
 ...
 * Đồng thời chạy lại khi chuyển sang bài viết khác.
 */
watch(
    () =>
        route.params.id ||
        route.params.maTinTuc,
    async () => {
      await getChiTietTinTuc();
...
/*
 * Phải dùng :deep vì nội dung được đưa vào
 * bằng v-html trong style scoped.
 */
