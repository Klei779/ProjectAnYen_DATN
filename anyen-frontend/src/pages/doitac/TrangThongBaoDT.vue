...
const loadThongBao = async () => {
  ...
  try {
    const response = await getThongBaoDoiTac();

    console.log("Dữ liệu thông báo API:", response);
    ...
    notifications.value = data.map(item => ({
      ...
      MaDoiTac: item.MaDoiTac ?? item.maDoiTac,
      ...
      TrangThaiThongBao:
          item.TrangThaiThongBao ?? item.trangThaiThongBao,
      ...
      DaDoc:
          ...
          item.DaDoc === 1 ||
          item.daDoc === 1,
      ThoiGianTao: item.ThoiGianTao ?? item.thoiGianTao
    }));
