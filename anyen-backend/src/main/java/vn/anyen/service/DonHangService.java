...
public class DonHangService {
        ...
        public DonHangResponse capNhatDonHang(
                ...
                // Nếu đơn hàng ở trạng thái Mới tạo (sau khi đối tác từ chối), tự động gửi lại
                // thông báo
        ...
        private List<ChiTietDonHang> hoanTonKhoVaXoaChiTietCu(DonHang donHang) {
                List<ChiTietDonHang> chiTietsCu = chiTietDonHangRepository.findByDonHang_MaDonHang(
        ...
        private BigDecimal taoChiTietDonHangVaTruTonKho(
                ...
                for (TaoDonHangRequest.SanPhamTrongDonRequest item : items) {
                        ...
                        if (chiTietsCu != null) {
                                for (ChiTietDonHang ct : chiTietsCu) {
                                        if (ct.getSanPham() != null && ct.getSanPham().getMaSanPham()
                                                        .equals(sanPham.getMaSanPham())) {
                                                if (ct.getSoLuong() != null && ct.getSoLuong().equals(soLuongDat)) {
                                                        if (ct.getTrangThaiDoiTac() != null
                                                                        && ct.getTrangThaiDoiTac() == 1) {
                                                                oldTrangThai = 1;
                                                        }
                                                }
                                                break;
        ...
        public DonHangResponse capNhatTrangThai(
                ...
                // Nếu đơn hàng ở trạng thái Mới tạo (sau khi đối tác từ chối), cho phép cập
                // nhật
                ...
                if (DonHang.TT_DANG_XU_LY.equals(trangThaiMoi)) {
                        thongBaoService.taoThongBaoDonHangDangXuLy(
                                        donHang.getMaDonHang());
                } else if (DonHang.TT_HOAN_THANH.equals(
                                trangThaiMoi)) {
        ...
        public DonHangResponse doiTacBaoDaGiao(
                ...
                List<ChiTietDonHang> chiTietsCuaDoiTac = chiTietDonHangRepository
                ...
                // 2 = đang xử lý
                // =========================================
                boolean dangXuLy = chiTietsCuaDoiTac.stream()
                                .allMatch(ct -> Integer.valueOf(2)
                                                .equals(
                ...
                // =========================================
                List<ChiTietDonHang> tatCaChiTiet = chiTietDonHangRepository
                                .findByDonHang_MaDonHang(
                                                maDonHang);

                boolean coDoiTacDaGiao = tatCaChiTiet.stream()
                                .anyMatch(ct -> Integer.valueOf(3)
                                                .equals(
                                                                ct.getTrangThaiDoiTac()));
                ...
                DonHang saved = donHangRepository.save(
                                donHang);
        ...
        public DonHangResponse thanhToanDonHang(
                ...
                if (!DonHang.TT_DA_GIAO.equals(donHang.getTrangThai()) 
                        && !Integer.valueOf(5).equals(donHang.getTrangThai())) {
                ...
                if (donHang.getPhuongThucThanhToan() == null || donHang.getPhuongThucThanhToan() == 0) {
                    donHang.setPhuongThucThanhToan(1); // 1 = Tiền mặt
                }
                ...
                // Tự động tạo hóa đơn tiền mặt nếu chưa có
                boolean daCoHoaDon = hoaDonRepository.existsByDonHang_MaDonHang(donHang.getMaDonHang());
                if (!daCoHoaDon) {
                    HoaDon hoaDon = HoaDon.builder()
                            .donHang(donHang)
                            .ngayIn(LocalDate.now())
                            .tongTien(donHang.getTongTien())
                            .phuongThucThanhToan(HoaDon.PT_TIEN_MAT)
        ...
        private DonHangResponse mapToDonHangResponse(DonHang donHang) {
                List<ChiTietDonHang> chiTiets = chiTietDonHangRepository.findByDonHang_MaDonHang(
                                donHang.getMaDonHang());
                ...
                List<DonHangResponse.ChiTietDonHangResponse> sanPhams = chiTiets.stream()
                ...
                return DonHangResponse.builder()
                                ...
                                .ngayGiaoDuKien(donHang.getNgayGiaoDuKien() != null
                                                ? donHang.getNgayGiaoDuKien().toString()
                                                : null)
        ...
        private DoiTacDonHangResponse mapToDoiTacDonHangResponse(
                ...
                for (int i = 0; i < chiTiets.size(); i++) {
                        ...
                        String tenSP = sp != null ? sp.getTenSanPham() : "Sản phẩm / Dịch vụ";
                        String tenDT = "Hệ thống An Yên";
                        if (sp != null && sp.getMaDoiTac() != null) {
                                tenDT = doiTacRepository.findById(sp.getMaDoiTac())
                                                .map(DoiTac::getTenDoiTac)
                                                .orElse("Hệ thống An Yên");
                        }
                        String loaiSP = (sp != null && sp.getLoai() != null) ? sp.getLoai() : "Dịch vụ";

                        sanPhams.add(
                                        DoiTacDonHangResponse.SanPhamTrongDonResponse.builder()
                                                        .stt(i + 1)
                                                        .maSanPham(sp != null ? sp.getMaSanPham() : null)
                                                        .ten(tenSP)
                                                        .tenSanPham(tenSP)
                                                        .tenDoiTac(tenDT)
                                                        .loai(loaiSP)
                                                        ...
                                                        .ngayGiaoDuKien(ct.getNgayGiaoDuKien() != null
                                                                        ? ct.getNgayGiaoDuKien().toString()
                                                                        : "")
                                                        .build());
                }
                ...
                // Kiểm tra hóa đơn
                java.util.Optional<HoaDon> hoaDonOpt = hoaDonRepository.findByDonHang_MaDonHang(donHang.getMaDonHang());
                ...
                return DoiTacDonHangResponse.builder()
                                ...
                                .maHoaDon(hoaDonOpt.map(HoaDon::getMaHoaDon).orElse(null))
                                .trangThaiHoaDon(hoaDonOpt
                                                .map(hd -> hd.getTrangThai() != null && hd.getTrangThai() == 1 ? "Đã in"
                                                                : "Chưa in")
                                                .orElse(null))
                                .ngayInHoaDon(hoaDonOpt
                                                .map(hd -> hd.getNgayIn() != null ? hd.getNgayIn().toString() : null)
                                                .orElse(null))
                                .trangThaiThanhToan(donHang.getTrangThaiThanhToan())
        ...
        public void xuLyDonHangDoiTac(
                ...
                // 1 = đã nhận
                // =========================================
                boolean daChapNhan = chiTiets.stream()
                                .allMatch(ct -> Integer.valueOf(1)
                                                .equals(
                                                                ct.getTrangThaiDoiTac()));
                ...
                // =========================================
                boolean coHopDong = hopDongRepository
                                .existsByDonHang_MaDonHang(
                                                maDonHang);

                boolean coQuanTai = donHangCoSanPhamQuanTai(
                                maDonHang);
                ...
                boolean coDoiTacDangXuLy = tatCaChiTiet.stream()
                                .anyMatch(ct -> ct.getTrangThaiDoiTac() != null
                                                &&
        ...
        private boolean tatCaDoiTacDaChapNhan(Integer maDonHang) {
                long tongDoiTac = donHangRepository.countDoiTacTrongDonHang(maDonHang);

                long soDoiTacDaChapNhan = donHangRepository.countDoiTacDaChapNhan(maDonHang);
        ...
        private void kiemTraKhongThemSanPhamCuaDoiTacTuChoi(
                ...
                for (TaoDonHangRequest.SanPhamTrongDonRequest item : items) {
                        if (maSanPhamsBiTuChoi.contains(item.getMaSanPham())) {
                                ...
                                throw new ResponseStatusException(
                                                HttpStatus.BAD_REQUEST,
                                                "Không thể thêm sản phẩm \"" + tenSanPham
                                                                + "\" vì sản phẩm này đã bị đối tác từ chối. Vui lòng chọn sản phẩm khác.");
                        }
                }
        }
        ...
        public void baoCaoSuCo(Integer maDonHang, String lyDoSuCo, Authentication authentication) {
                ...
                // Kiểm tra trạng thái hiện tại - không thể báo cáo sự cố nếu đã hủy hoặc hoàn
                // thành
                if (DonHang.TT_DA_HUY.equals(donHang.getTrangThai()) ||
                        ...
                        throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                                        "Đơn hàng không thể báo cáo sự cố ở trạng thái hiện tại");
                }
                ...
                try {
                        ...
                        // Không throw lỗi nếu tạo thông báo thất bại, đơn hàng vẫn đã được báo cáo sự
                        // cố
        ...
        public void giaiQuyetSuCo(Integer maDonHang, Authentication authentication) {
                ...
                if (!DonHang.TT_GAP_SU_CO.equals(donHang.getTrangThai())) {
                        throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                                        "Đơn hàng không ở trạng thái gặp sự cố");
                }

                if (donHang.getTrangThaiTruocSuCo() == null) {
                        throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
                                        "Không tìm thấy trạng thái trước khi gặp sự cố");
                }
